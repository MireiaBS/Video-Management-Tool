...
function moviesAverageOfDirector(movies, director) {
  ...
  let result = average.reduce((acc, item) => {
    return (acc = (acc + item) / films.length);
  });
...
function orderByYear(movies) {
  let newArray = [...movies];
  let result = newArray.sort(SortArray).sort((a, b) => a.year - b.year);
...
function moviesAverageByCategory(movies, genre) {  
  ...
  let allFilms = movies.filter(movie => movie.genre.includes(genre))
  .filter( movie => movie.score != '')
  .map((average) => average.score);
  
  let result = allFilms.reduce((acc, item) => { return (acc + item)}) / allFilms.length;
...
function hoursToMinutes(movies) {
  let movies2 = movies.map((item) => { return { ...item } });
  ...
  function myFunction(movie) {
    ...
    if (parts.length > 2) {
      ...
      movie.duration =
        Number(replaced[0]) * 60 + Number(replaced[2] + replaced[3]);
    } else {
